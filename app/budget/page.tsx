...
export default function BudgetScreen() {
  ...
  const loadExpenses = async () => {
    ...
    try {
      ...
      if (error) { console.error('Error loading expenses:', error); setLoading(false); return; }
      setExpenses(data || []);
      setLoading(false);
    } catch (err) {
  ...
  const handleSubmitDepense = () => { loadExpenses(); };

  const totalRevenus = expenses.filter(e => e.type === 'revenu').reduce((sum, e) => sum + e.amount, 0);
  const totalDepenses = expenses.filter(e => e.type === 'depense').reduce((sum, e) => sum + e.amount, 0);
  const solde = totalRevenus - totalDepenses;

  // Catégories calculées dynamiquement depuis les vraies dépenses
  const categoriesMap: Record<string, number> = {};
  expenses.filter(e => e.type === 'depense').forEach(e => {
  ...
  const categoryColors = ['#EF4444', '#F59E0B', '#A78BFA', '#22D3EE', '#10B981'];

  const iconMap: { [key: string]: any } = {
    'Nourriture': ShoppingCart, 'Sorties': Coffee, 'Loisirs': Film,
    'Restaurant': Utensils, 'default': ShoppingCart,
  };
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] pb-24">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <h1 className="text-[32px] font-bold text-[#F8FAFC] mb-8">Budget</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-[#1E293B] rounded-2xl p-6 border-l-4 border-[#10B981]">
                ...
                <p className="text-[40px] font-bold text-[#10B981] leading-none">{totalRevenus.toFixed(0)}€</p>
                <p className="text-sm text-[#94A3B8] mt-2">Total</p>
              </div>
              ...
              <div className="bg-[#1E293B] rounded-2xl p-6 border-l-4 border-[#EF4444]">
                ...
                <p className="text-[40px] font-bold text-[#EF4444] leading-none">{totalDepenses.toFixed(0)}€</p>
                <p className="text-sm text-[#94A3B8] mt-2">Total</p>
              </div>
            </div>
            ...
            <div className="bg-[#1E293B] rounded-2xl p-6 border border-[#334155]">
              <h3 className="text-lg font-semibold text-[#F8FAFC] mb-6">Résumé</h3>
              <div className="space-y-4">
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm text-[#94A3B8]">Revenus</span>
                    <span className="text-sm font-semibold text-[#10B981]">{totalRevenus.toFixed(0)}€</span>
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm font-semibold text-[#EF4444]">{totalDepenses.toFixed(0)}€</span>
                  </div>
                  <div className="h-3 bg-[#0F172A] rounded-full overflow-hidden">
                    <div className="h-full bg-[#EF4444] rounded-full transition-all"
                      style={{ width: `${totalRevenus > 0 ? Math.min(100, (totalDepenses / totalRevenus) * 100) : 0}%` }} />
                  </div>
                </div>
                <div className="pt-4 border-t border-[#334155]">
                  <div className="flex items-center justify-between">
                    <span className="text-base font-semibold text-[#F8FAFC]">Solde</span>
                    <span className={`text-2xl font-bold ${solde >= 0 ? 'text-[#10B981]' : 'text-[#EF4444]'}`}>
            ...
            {/* TRANSACTIONS */}
            <div>
              <h2 className="text-xl font-bold text-[#F8FAFC] mb-4">Transactions</h2>
              {loading && <p className="text-sm text-[#64748B] text-center py-4">Chargement...</p>}
              {!loading && transactions.length === 0 && (
                <p className="text-sm text-[#64748B] text-center py-8">Aucune transaction</p>
              )}
              <div className="space-y-3">
                {transactions.map((transaction) => {
                  ...
                  return (
                    <div key={transaction.id} className="bg-[#1E293B] rounded-xl p-4 border border-[#334155] hover:border-[#22D3EE]/50 transition-colors">
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isRevenu ? 'bg-[#10B981]/20' : 'bg-[#EF4444]/20'}`}>
                          <Icon size={20} className={isRevenu ? 'text-[#10B981]' : 'text-[#EF4444]'} />
                        </div>
                        <div className="flex-1">
                          <h4 className="text-base font-semibold text-[#F8FAFC] mb-1">{transaction.title}</h4>
                        ...
                        <span className={`text-xl font-bold ${isRevenu ? 'text-[#10B981]' : 'text-[#EF4444]'}`}>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-[#22D3EE] to-[#1DB8D1] rounded-2xl p-6 text-[#0F172A]">
              ...
              <p className="text-sm opacity-80">Toutes périodes</p>
            </div>

            {/* CATÉGORIES DYNAMIQUES */}
            <div className="bg-[#1E293B] rounded-xl p-5 border border-[#334155]">
              <h3 className="text-base font-semibold text-[#F8FAFC] mb-4">Dépenses par catégorie</h3>
              {topCategories.length === 0 ? (
                <p className="text-sm text-[#64748B] text-center py-4">Aucune dépense</p>
              ) : (
                <div className="space-y-3">
      ...
      <ModalNouvelleDepense isOpen={modalDepenseOpen} onClose={() => setModalDepenseOpen(false)} onSubmit={handleSubmitDepense} />
    </div>
  );
}
